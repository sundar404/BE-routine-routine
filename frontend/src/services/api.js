...
export const teachersAPI = {
  ...
  exportTeacherScheduleToPDF: (id, semesterGroup = null) => {
    const params = semesterGroup ? { semesterGroup } : {};
    return queuedRequest(
      () => api.get(`/routines/teacher/${id}/export-pdf`, { 
        responseType: 'blob',
        params 
      }),
      `exporting schedule to PDF for teacher ${id} (${semesterGroup || 'all'} semesters)`
    );
  },

  exportAllTeachersSchedulesToPDF: (semesterGroup = 'all') => {
    const params = semesterGroup && semesterGroup !== 'all' ? { semesterGroup } : {};
    return queuedRequest(
      () => api.get('/routines/teachers/export-pdf', { 
        params,
        responseType: 'blob' 
      }),
...
export const roomsAPI = {
  ...
  // PDF Export Methods (New)
  exportRoomScheduleToPDF: (roomId, semesterGroup = null) => {
    const params = semesterGroup ? { semesterGroup } : {};
    return api.get(`/routines/room/${roomId}/export-pdf`, {
      responseType: 'blob',
      params
    });
  },
  exportAllRoomSchedulesToPDF: (semesterGroup = 'all') => {
    const params = semesterGroup && semesterGroup !== 'all' ? { semesterGroup } : {};
    return api.get('/routines/rooms/export-pdf', {
      params,
      responseType: 'blob'
    });
  }
