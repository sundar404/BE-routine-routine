...
async function testTeacherRoutineFiltering() {
  ...
  const teacherId = '68681d200bef564017898160'; // Anish Adhikari
  
  try {
    // Test 1: Get full teacher routine (all semesters)
    console.log('1. Fetching teacher schedule (all semesters)...');
    const allResponse = await axios.get(`${baseURL}/api/teachers/${teacherId}/schedule`);
    
    if (allResponse.data?.data?.routine) {
      console.log('✓ Successfully fetched teacher schedule');
      
      // Count total classes
      let totalClassCount = 0;
      Object.values(allResponse.data.data.routine).forEach(daySlots => {
      ...
      let sampleClasses = [];
      for (const [dayKey, daySlots] of Object.entries(allResponse.data.data.routine)) {
    ...
    const routineData = allResponse.data.data;
